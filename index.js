...
AI ENGINE:        Google Gemini (Flash Model) with automatic caching.
...
Ensure 'package.json' includes: express, cors, pg, mongoose, dotenv, @google/generative-ai, express-rate-limit
...
// ---------------------------------------------------------------------
await client.query(CREATE TABLE IF NOT EXISTS leaderboard ( id SERIAL PRIMARY KEY, username VARCHAR(50) NOT NULL, score INTEGER NOT NULL, difficulty VARCHAR(20) NOT NULL, ip_address VARCHAR(45), created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP ););

// ---------------------------------------------------------------------
// TABLE 2: CERTIFICATE REQUESTS (Admin)
// Queue for generating completion certificates.
// ---------------------------------------------------------------------
await client.query(CREATE TABLE IF NOT EXISTS certificate_requests ( id SERIAL PRIMARY KEY, username VARCHAR(100) NOT NULL, score INTEGER NOT NULL, status VARCHAR(20) DEFAULT 'Pending', request_date TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP ););
...
Force Default Metadata (Topic/Difficulty) if missing (The "Workaround").

Check MongoDB Cache (25% Probability).

If Cache Miss -> Call Google Gemini AI.

Save AI result to Cache for future use.

Return Problem to Client.
*/
